...
-- Paste into Data Studio as a Custom Query data source.
...
m5 AS (SELECT channel, 'Time Decay' AS model, SUM(EXP(-0.00412*h)/NULLIF(SUM(EXP(-0.00412*h)) OVER (PARTITION BY user_pseudo_id, conversion_id),0)) AS c FROM js GROUP BY 1),

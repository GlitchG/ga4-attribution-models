-- User Path Analysis
-- Shows the most common conversion paths (sequence of channel touchpoints leading to purchase)
...
user_journey AS (
  SELECT
    c.user_pseudo_id,
    c.conversion_id,
    STRING_AGG(s.channel ORDER BY s.session_start_micros LIMIT 20 SEPARATOR ' → ') AS conversion_path,

...
- Extracts sessions from `session_start` events using `event_params` source/medium (correct session-level scope, not user-level `traffic_source`)
...
- **Session-based touchpoints** — source/medium from `event_params` on `session_start`, not user-level `traffic_source`
...
| Any date | `event_params` extraction on `session_start` events | The approach used in all queries here. Works on ALL exports including the public sample. |

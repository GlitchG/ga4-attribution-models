...
### Cleanup
- **Date defaults aligned across all three layers.** `workflow_settings.yaml`, `includes/constants.js`, and every `DECLARE start_date / end_date` in `standalone-sql/**/*.sql` now agree on `20201101` → `20210131` (the full GA4 public-sample window). Previously, the Dataform pipeline ran one window while `constants.js` (used as a fallback by some code paths) carried a narrower `20201220` default, producing different row counts depending on the entry point.
- **`paths_dashboard` materialised as a table.** Was `type: "view"`, so every Data Studio chart refresh re-executed the `STRING_AGG` over the underlying ~12k journeys. Now `type: "table"` — chart-refresh cost is effectively zero, daily rebuild cost is unchanged.
- **CI `dry-run` job removed.** It was gated by `if: ${{ secrets.GCP_SERVICE_ACCOUNT_KEY != '' }}` and the secret had never been set, so the job had silently never executed. Better to delete the aspirational job than imply CI coverage that doesn't exist. The `lint-sql` job (which does run on every PR) is unchanged.
- **`docs/DATA_STUDIO_GUIDE.md` structural slim.** Merged the old §7 (Revenue Attribution Sankey) and §8 (Cross-Model Revenue Comparison) into a single §7 "Cross-Model Channel View" with five chart sub-recipes (stacked bar, 100% stacked, variance pivot, radar, Sankey-via-community-viz). Renumbered §9–13 → §8–12 to close the gap. Net: ~80 lines shorter, no content lost, less scrollback.

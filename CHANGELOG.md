...
### Fixed
- **`attribution_with_roas.sqlx`** — removed orphaned SELECT block and duplicate `cost_daily` CTE from copy-paste artifact. Fixed `source_system` → `source_systems` column reference. File now compiles when cost module is enabled.

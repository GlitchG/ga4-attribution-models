...
-- Session-level source/medium from event_params. Channel normalization applied.
...
td  AS (SELECT channel, SUM(EXP(-0.1 * hours_before) / NULLIF(SUM(EXP(-0.1 * hours_before)) OVER (PARTITION BY user_pseudo_id, conversion_id), 0)) AS w FROM journey_sessions GROUP BY 1),

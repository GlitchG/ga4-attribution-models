...
-- Ignores direct/none traffic (source = '(direct)' OR medium = '(none)')
...
all_events AS (
  ...
  FROM `bigquery-public-data.ga4_obfuscated_sample_ecommerce.events_*`
  WHERE _TABLE_SUFFIX BETWEEN start_date AND end_date
    -- Exclude direct/none traffic
    AND NOT (
      (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'source') IS NULL
      OR (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'source') = '(direct)'
      OR (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'medium') = '(none)'
    )
),

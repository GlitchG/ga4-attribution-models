...
-- Uses ARRAY subquery to filter Direct sessions from the reversed path.
-- Fallback: IFNULL(result, 'Direct') when all sessions are Direct.
...
journeys AS (
  SELECT
    ...
    c.conversion_id,
    ARRAY_AGG(
      STRUCT(s.channel, s.session_start_micros)
...
last_non_direct AS (
  SELECT
    user_pseudo_id,
    conversion_id,
    -- ARRAY subquery: scan reversed path, pick first non-Direct
    IFNULL(
      ARRAY(
        SELECT channel
        FROM UNNEST(ARRAY_REVERSE(path))
        WHERE channel != 'Direct'
        LIMIT 1
      )[SAFE_OFFSET(0)],
      'Direct'  -- Fallback: all sessions were Direct
